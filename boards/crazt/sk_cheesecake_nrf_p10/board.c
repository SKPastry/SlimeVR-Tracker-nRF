...
#define ECLK_EN_GPIO_PIN DT_GPIO_PIN(ZEPHYR_USER_NODE, eclk_en_gpios)
#define ECLK_EN_GPIO_PORT_NUM DT_PROP(DT_GPIO_CTLR(ZEPHYR_USER_NODE, eclk_en_gpios), port)
#define ECLK_EN_GPIO NRF_GPIO_PIN_MAP(ECLK_EN_GPIO_PORT_NUM, ECLK_EN_GPIO_PIN)
...
{
	nrf_gpio_cfg_default(ECLK_EN_GPIO);
...
static int board_sk_cheesecake_nrf_p10_init(void)
{
	nrf_gpio_cfg_default(ECLK_EN_GPIO);
	board_output_off(HEAT_EN_GPIO);
	board_output_off(LED_EN_GPIO);
...
SYS_INIT(board_sk_cheesecake_nrf_p10_init, PRE_KERNEL_1, CONFIG_KERNEL_INIT_PRIORITY_DEFAULT);
