...
static void power_thread(void)
{
	...
	while (1)
	{
		...
		bool raw_device_plugged = plug_gpio_active || charging || charged || plugged || usb_plugged || pmic_plugged;
		...
		power_battery_set_charged(charged || (plug_gpio_active && !charging));
		...
		} else if (plug_gpio_active || plugged || usb_plugged || pmic_plugged) {
			system_led_pattern = SYS_LED_PATTERN_PULSE_PERSIST;
		} else if (battery_low) {
